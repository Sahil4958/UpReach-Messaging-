...
const UsingForm = () => {
  ...
  // Function to handle API call for uploading contacts
  const uploadContacts = async (contacts) => {
    ...
    try {
      const response = await fetch(`${config.apiBaseUrl}/api/upload-form`, {
        method: "POST",
        headers: {
      ...
      console.log(
        ...
        JSON.stringify({ contacts: [formData] })
      );
      ...
      if (response.ok) {
        ...
        if (
          ...
          alert(data.message); // Show alert for limit errors
        }
        //  else {
        //   toast.error(data.message || "Failed to upload contact.");
        // }
        return false;
      }
    } catch (error) {
    ...
    if (validateForm()) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // **Call uploadContacts after saving the contact**
          console.log("Sending contacts:", [...contactList, formData]);
